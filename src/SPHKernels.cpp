...
SPHKernels::SPHKernels(float r)
{
	...
	visc_const = 5 / (pi * pow(kernel_radius, 6));
	st = 100;
}
...
void SPHKernels::pressure(Particle* p1, Particle* p2) {
	...
	float yDistance = p1->getY() - p2->getY();
	float distanceSquared = xDistance * xDistance + yDistance * yDistance;
	if (kernel_radius_squared > distanceSquared && distanceSquared != 0) {
...
void SPHKernels::surface_tension(Particle* p1, Particle* p2) {
	...
	if (kernel_radius_squared > distanceSquared && distanceSquared != 0) {
		...
		float surface_force = st*(p1->getMass() / p1->getDensity()) * poly_6_lap_const * (distanceSquared-kernel_radius_squared)*(5*distanceSquared-kernel_radius_squared)* n / std::abs(n);
		...
		surface_force = st*(p2->getMass() / p2->getDensity()) * poly_6_lap_const * (distanceSquared - kernel_radius_squared)*(5 * distanceSquared - kernel_radius_squared)* n / std::abs(n);
